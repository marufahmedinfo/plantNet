...
async function run() {
  ...
  try {
    ...
    // update a plant from db by seller
    app.put('/plants/:id', verifyToken, verifySeller, async (req, res) => {
      ...
      const updatePlants = {
        $set: {
          name: updatedPlants.name,
          category: updatedPlants.category,
          description: updatedPlants.description,
          price: updatedPlants.price,
          quantity: updatedPlants.quantity,
          image: updatedPlants.image,
        }
      };
      const result = await plantsCollection.updateOne(filter, updatePlants, options);
      res.send(result)
    });
    ...
    app.post('/order', verifyToken, async (req, res) => {
      ...
      if (result?.insertedId) {
        ...
        sendEmail(orderInfo?.customer?.email, {
          ...
          message: `You've placed an successfully. Transaction Id: ${result?.insertedId}`,
        })
        // To seller
        ...
        .aggregate([
...

          {
            $lookup: {
              ...
              localField: 'plantId',  // local data that name of that you want to match
              foreignField: '_id',   //foreign field name of that same data
              as: 'plants',   // return tha data as plants array (array naming)
            },
          },
